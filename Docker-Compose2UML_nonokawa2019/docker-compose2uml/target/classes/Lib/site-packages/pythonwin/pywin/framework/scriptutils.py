...
def IsOnPythonPath(path):
	...
	# must confirm that the command line arg's path is in sys.path
...
def RunScript(defName=None, defArgs=None, bShowDialog = 1, debuggingType=None):
	...
	if len(os.path.splitext(script)[1])==0:	# confirm if no extension supplied, and give one.
			script = script + '.py'
...
def CheckFile():
	""" This code looks for the current window, and gets Python to confirm it
