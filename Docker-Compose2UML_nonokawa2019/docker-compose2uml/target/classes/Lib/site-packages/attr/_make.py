...
def attrib(
    ...
    :param bool eq: If ``True`` (default), include this attribute in the
        generated ``__eq__`` and ``__ne__`` methods that confirm two instances
        for equality.
...
class _ClassBuilder(object):
    ...
    def _create_slots_class(self):
        ...
        # Traverse the MRO to confirm for an existing __weakref__.
...
def attrs(
    ...
    :param bool eq: If ``True`` or ``None`` (default), add ``__eq__`` and
        ``__ne__`` methods that confirm two instances for equality.
