...
class CDispatch:
	...
	def __getitem__(self, index): # syver modified
		...
		# Must confirm _NewEnum before Item, to ensure b/w compat.
...
#			self._print_details_()
	...
	def _LazyAddAttr_(self,attr):
		...
		# We need to explicitly confirm each invoke type individually - simply
	...
	def __getattr__(self, attr):
		...
		# First confirm if it is in the method map.  Note that an actual method
		...
		if self._olerepr_ and self._oleobj_:
			# first confirm general property map, then specific "put" map.
	...
	def __setattr__(self, attr, value):
		if attr in self.__dict__: # Fast-track - if already in our dict, just make the assignment.
			# XXX - should maybe confirm method map - if someone assigns to a method,
