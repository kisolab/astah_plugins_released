...
# Rather than confirm for ".zip" or other tricks, PEP302 defines
# a "__loader__" attribute, so we use that.
# (Later, it may become necessary to confirm if the __loader__ can update files,
...
def GetModuleForTypelib(typelibCLSID, lcid, major, minor):
	...
	# module to our cache though - confirm that here.
...
def EnsureModule(typelibCLSID, lcid, major, minor, progressInstance = None, bValidateFile=not is_readonly, bForDemand = bForDemandDefault, bBuildHidden = 1):
	...
	Given the IID, LCID and version information for a type library, confirm and if
	...
	try:
		try:
			...
			try:
				...
				# if the above line doesn't throw a pythoncom.com_error, confirm if
	...
	if module is None:
		...
		# can't/don't want to do this - so before giving up, confirm for
