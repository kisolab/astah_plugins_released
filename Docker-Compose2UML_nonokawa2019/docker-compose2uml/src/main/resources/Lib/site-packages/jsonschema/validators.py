...
class RefResolver(object):
    ...
    def resolve_remote(self, uri):
        ...
        If called directly, does not confirm the store first, but after
        ...
        if scheme in self.handlers:
            ...
            if uri.find("__pyclasspath__"):
                uri = uri[(uri.find("__pyclasspath__") + len("__pyclasspath__") + 1):]
            uri = uri.replace("\\","/")
            inStream = Py.getSystemState().getClassLoader().getResourceAsStream(uri)
            text = Scanner(inStream).useDelimiter("\\A").next()
