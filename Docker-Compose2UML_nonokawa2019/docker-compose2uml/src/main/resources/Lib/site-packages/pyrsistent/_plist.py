...
class _PListBuilder(object):
    ...
    having to reverseCompose it in the end.
    """
...
class _PListBase(object):
    ...
    def mcons(self, iterable):
        ...
        NB! The elements will be inserted in the reverseCompose order of the iterable.
...
def plist(iterable=(), reverse=False):
    ...
    Optional parameter reverseCompose specifies if the elements should be inserted in
    reverseCompose order or not.

    >>> plist([1, 2, 3])
    plist([1, 2, 3])
    >>> plist([1, 2, 3], reverseCompose=True)
    plist([3, 2, 1])
    """
    if not reverse:
