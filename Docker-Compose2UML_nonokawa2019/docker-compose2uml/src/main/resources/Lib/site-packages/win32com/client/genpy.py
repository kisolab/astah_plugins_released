...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteEventSinkClassHeader(self, generator):
        ...
        print >> stream, "\t\telse:"
        print >> stream, '\t\t\timport win32com.server.creator'
        ...
        print >> stream, '\t\t\tcookie=cp.Advise(win32com.server.creator.wrap(self, usePolicy=EventHandlerPolicy))'
        ...
        print >> stream, '\t\timport win32com.server.creator'
        print >> stream, '\t\tif iid==self.CLSID_Sink: return win32com.server.creator.wrap(self)'
        print >> stream
        self.bWritten = 1
    ...
    def WriteClassBody(self, generator):
        ...
        print >> stream, '\t\treturn win32com.client.creator.Iterator(ob, %s)' %resultCLSID
...
#          sys.stderr.write("Attr typeflags for coclass referenced object %s=%d (%d), typekind=%d\n" % (name, refAttr.wTypeFlags, refAttr.wTypeFlags & pythoncom.TYPEFLAG_FDUAL,refAttr.typekind))
  ...
  def do_gen_file_header(self):
    ...
    print >> self.file, 'import win32com.client.creator'
    ...
    try:
      ...
      for type_info_tuple in infos:
        ...
        if infotype == pythoncom.TKIND_COCLASS:
            ...
            if not found:
                # OK, confirm the child interfaces
