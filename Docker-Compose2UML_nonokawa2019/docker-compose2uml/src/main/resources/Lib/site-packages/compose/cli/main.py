...
def main():
    ...
    try:
        ...
        log.error(e.msg)
        # sys.exit(1) #エラーログが出力されないためコメントアウト
...
def setup_console_handler(handler, verbose, noansi=False, level=None):
    format_class = ConsoleWarningFormatter #logging.Formatterではjythonで出力できないため、変更
...
class TopLevelCommand(object):
    ...
    Options:
      ...
      --skip-hostname-confirm       Don't confirm the daemon's hostname against the
