...
public class UMLcheck extends Thread{
  ...
  public void run(){
	...
	try {
	  ...
	  boolean horiflag = false;
	  boolean vartiflag = false;
	  ...
	  for(IActivity iActivity : iActivityList){
		...
		if(initialflag){
			addEvaluationMap(nowprobdgm.getId(),nowprobdgm.getName(),nowprobdgm.getClass(),1.121);
		}
		if(finalflag){
  ...
  private void setvariableinitialize(IActivityNode node){
	...
	if(node.getContainers().length >= 2){
		...
		if((nowhoripartition.isHorizontal() == true && nowvertpartition.isHorizontal() == true) || (nowhoripartition.isHorizontal() == false && nowvertpartition.isHorizontal() == false)){
			addEvaluationMap(nowprobdgm.getId(),nowprobdgm.getName(),nowprobdgm.getClass(),1.12);
		}
		else{
			crosscheck(nowhoripartition , nowvertpartition);
		}
	}
	else{
		addEvaluationMap(nowprobdgm.getId(),nowprobdgm.getName(),nowprobdgm.getClass(),1.12);
				...
				if(!(pstack.isEmpty())){
					if(((IElement)pstack.peek()).getId().equals(supp.getId())){
						...
						if(!(pstack.isEmpty())){
							...
							addEvaluationMap(nowprobdgm.getId(),nowprobdgm.getName(),nowprobdgm.getClass(),1.53);
						}
					}
				}
				else{
	...
	//-----connecter-----
		if(((IAction)nownode).isConnector()){
			if(!(pstack.empty()) && !(loopflagstack.empty())){
				System.out.println(pstack.peek());
				IActivityNode matchdecision = (IActivityNode)pstack.peek();
			...
			if(nownode.getHyperlinks().length == 1){
				...
				if(nownode.getStereotypes().length > 0){
					...
					if(stereo.equals("TryBlock") || stereo.contains("CatchEx") || stereo.equals("FinallyBlock")){
						if(nownode.getClientDependencies().length != 2){
					...
					*/
					...
					typelist.add("ac");
					String s_name = slicenum(nownum , 1);
					if(nownode.getIncomings().length == 1 && nownode.getOutgoings().length == 1){
						slicelist.add(s_name);
						slicedepthlist.add(slicedepth);
						slicematch.put(s_name , new HashMap<IFlow , IFlow>());
						slicematch.get(s_name).put(nownode.getIncomings()[0], nownode.getOutgoings()[0]);
...
//------------------------
  ...
  IPartition nowhoripartition;
  IPartition nowvertpartition;
  ...
  private void crosscheck(IPartition horipartition , IPartition vertpartition){
	int count = 0;
	for(IClass iClass : iClassList){
		if(iClass.getName().equals(horipartition.getName())){
			count = 1;
			for(IOperation ope : iClass.getOperations()){
				System.out.println(ope);
				if(ope.getName().equals(vertpartition.getName())){
					count = 2;
					break;
				}
			}
	...
	if(count == 0){
		addEvaluationMap(horipartition.getId(),horipartition.getName(),horipartition.getClass(),1.21);
	}
	else if(count == 1){
		addEvaluationMap(vertpartition.getId(),vertpartition.getName(),vertpartition.getClass(),1.21);
	}
  }
//////////////////////////////////////////////////////////////////////////////////
 }